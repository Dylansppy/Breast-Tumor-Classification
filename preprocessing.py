...
TRAIN_IMAGE_DIR = os.path.join('..', 'data', 'breast', 'Train')
TRAIN_IMAGE_PROCESSED = os.path.join('..', 'data', 'train_processed')

TEST_IMAGE_DIR = os.path.join('..', 'data', 'breast', 'Test')
TEST_IMAGE_PROCESSED = os.path.join('..', 'data', 'test_processed')
...
if __name__ == '__main__':
    ...
    for i in range(4):
        p.apply_async(preprocessing,
                      args=(os.path.join(TRAIN_IMAGE_DIR),
                            os.path.join(TRAIN_IMAGE_PROCESSED, magnification_factors[i]),
                            magnification_factors[i])
                      )
        p.apply_async(preprocessing,
                      args=(os.path.join(TEST_IMAGE_DIR),
                            os.path.join(TEST_IMAGE_PROCESSED, magnification_factors[i]),
                            magnification_factors[i])
                      )
